...
class DocumentClassifier:
    ...
    def clean_tokens(self, tokens):
        ...
        tokens = [token for token in tokens if token not in self.stopwords_lang["spanish"] and 
                  token not in self.stopwords_lang["english"] and token not in self.stopwords_lang["comun"]]
        return tokens

    # Eliminar bloques de código anidados, tablas, y código entre llaves
    def preprocesing_prev(self, text):
    ...
    def process_file_for_filter(self, file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            markdown_content = file.read()
        # Preprocesamiento
        content = self.preprocesing_prev(markdown_content)
        # Eliminar dígitos
        ...
        word_frequencies = {lang: Counter() for lang in self.common_words.keys()}
        unique_words_per_lang = {lang: set() for lang in self.common_words.keys()}
        
        for segment in segments:
            if segment.strip():
                words = word_tokenize(segment)
                words = [token.lower() for token in words if token.isalnum()]
                lang_scores = {lang: sum(word in words for word in common) for lang, common in self.common_words.items()}
                detected_lang = max(lang_scores, key=lang_scores.get)
                language_counts[detected_lang] += 1
                filtered_words = [word for word in words if word not in self.stopwords_lang.get(detected_lang, set())]
                
                tagged_tokens = pos_tag(filtered_words)
                lemmatized_words = {self.lemmatizer.lemmatize(word) for word, tag in tagged_tokens}
        ...
        if total_unique_words > 0:
            ...
            language_percentages = {lang: 0 for lang in self.common_words.keys()}
        
        return language_percentages, word_frequencies

    # Filtrado de archivos para todo el directorio
    def filter_files(self, directory_path):
        less_than_5_files = []
        more_than_5_files = []
        ...
        for root, dirs, files in os.walk(directory_path):
            for filename in files:
                if filename.endswith(".md"):
                    ...
                    language_percentages, _ = self.process_file_for_filter(file_path)
    ...
    # Función para clasificación de idioma en español usando WordNet
    def is_spanish_word(self, word):
    ...
    def classify_words(self, tokens):
        ...
        for token in tokens:
            english = self.is_english_word(token)
            spanish = self.is_spanish_word(token)
    ...
    def process_file_main(self, file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            text = file.read()
        # Preprocesado
        text = self.preprocesing_prev(text)
        text = self.preprocesing(text)
        tokens = nltk.word_tokenize(text)
        tokens = self.clean_tokens(tokens)
        # Uso de la función "classify_words" para clasificar cada documento
        english_words, spanish_words = self.classify_words(tokens)
        # Lematización
        english_lemmatized = [self.nlp_en(word)[0].lemma_ for word in english_words]
        spanish_lemmatized = [self.nlp_es(word)[0].lemma_ for word in spanish_words]
    ...
    def process_multiple_files(self, filtered_files):
        ...
        for file_path in filtered_files:
            english_percentage, spanish_percentage = self.process_file_main(file_path)
...
if __name__ == "__main__":
    ...
    less_than_5_files, more_than_5_files = classifier.filter_files(directory_path)

    # Procesar los archivos que tienen el idioma de menor proporción superior al 5%
    count25, count25_40, count40_50, files_matching_condition25_40, files_matching_condition40_50 = classifier.process_multiple_files(more_than_5_files)
